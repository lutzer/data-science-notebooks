...
- **Method:** take the latest available year of `edindex` per `GDLCODE`, join onto GDL polygons by `gdlcode` (case-normalised), drop polygons without a value, and rasterise via `dominant_region_mask` onto the shared 0.5° grid — same helper `03_region_mask.ipynb` uses for Natural Earth countries. The mask oversamples each atlas cell into 10×10 sub-pixels via `rasterio.features.rasterize` and takes the mode, which handles ~1 800 polygons in a fraction of a second regardless of region count. Values are already `[0, 1]` with higher = better, so **no sign inversion**. Ocean is masked via `is_land` from `grid.nc`.
